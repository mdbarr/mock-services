'use strict';

const errors = require('restify-errors');

function Webhook (mock) {
  this.config = mock.config.webhook;
  this.log = mock.log.child({ service: 'webhook' });

  this.queues = {};

  mock.api.post('/api/webhook/:any', (req, res, next) => {
    if (!req.body || !req.body.webhookId) {
      return next(new errors.BadRequestError(new Error('Invalid webhook event payload')));
    }

    const event = {
      id: mock.utils.generateUniqueId(24),
      webhookId: req.body.webhookId,
      eventType: req.body.event,
      method: req.method,
      url: req.url,
      headers: req.headers,
      event: req.body,
      timestamp: mock.utils.timestamp(),
    };

    if (!this.queues[event.webhookId]) {
      this.queues[event.webhookId] = [ ];
    }

    this.queues[event.webhookId].push(event);
    this.log.info(`Received eventType=${ event.eventType } on webhookId=${ event.webhookId }`);

    res.send(200, {
      id: event.id,
      ok: true,
    });
    return next();
  });

  mock.api.get('/api/webhook/:webhookId/clear', (req, res, next) => {
    if (this.queues[req.params.webhookId]) {
      this.queues[req.params.webhookId] = [];
      res.send(204);
      return next();
    }

    return next(new errors.NotFoundError(`${ req.params.webhookId } not found`));
  });

  mock.api.get('/api/webhook/:webhookId/:eventType', (req, res, next) => {
    if (this.queues[req.params.webhookId]) {
      const event = this.queues[req.params.webhookId].find(w => w.eventType === req.params.eventType);
      if (event) {
        res.send(200, event);
        return next();
      }
      return next(new errors.NotFoundError(`${ req.params.eventType } not found`));
    }

    return next(new errors.NotFoundError(`${ req.params.webhookId } not found`));
  });

  mock.api.get('/api/webhook/:webhookId/:eventType/all', (req, res, next) => {
    if (this.queues[req.params.webhookId]) {
      const events = this.queues[req.params.webhookId].filter(w => w.eventType === req.params.eventType);
      if (events) {
        res.send(200, events);
        return next();
      }
      return next(new errors.NotFoundError(`${ req.params.eventType } not found`));
    }

    return next(new errors.NotFoundError(`${ req.params.webhookId } not found`));
  });

  mock.api.get('/api/webhook/:webhookId/:eventType/clear', (req, res, next) => {
    if (this.queues[req.params.webhookId]) {
      this.queues[req.params.webhookId] = this.queues[req.params.webhookId].filter(w => w.eventType !== req.params.eventType);
      res.send(204);
      return next();
    }

    return next(new errors.NotFoundError(`${ req.params.webhookId } not found`));
  });

  mock.api.get('/api/webhook/:webhookId/:eventType/latest', (req, res, next) => {
    if (this.queues[req.params.webhookId]) {
      const event = this.queues[req.params.webhookId].findLast(w => w.eventType === req.params.eventType);
      if (event) {
        res.send(200, event);
        return next();
      }
      return next(new errors.NotFoundError(`${ req.params.eventType } not found`));
    }

    return next(new errors.NotFoundError(`${ req.params.webhookId } not found`));
  });

  this.start = async () => {
    this.interval = setInterval(() => {
      this.log.info('Clearing stale webhooks...');
      for (const id in this.queues) {
        this.queues[id] = this.queues[id].
          filter(w => w.timestamp > mock.utils.timestamp() - mock.utils.milliseconds('8h'));
      }
    }, mock.utils.milliseconds('5m'));

    this.log.info('Mock Webhook Server ready!');
  };
}

module.exports = (mock) => new Webhook(mock);
