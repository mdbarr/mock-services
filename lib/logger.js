...
module.exports = (config = {}) => {
  ...
  logger.req = (req, res, next) => {
    ...
    if (req.params) {
      logger.silly(`params ${ JSON.stringify(req.params, null, 2) }`);
    }
    if (req.query) {
      logger.debug(`query ${ JSON.stringify(req.query, null, 2) }`);
    }
    if (req.authorization) {
      logger.silly(`authorization ${ JSON.stringify(req.authorization, null, 2) }`);
    }
    if (req.headers) {
      logger.silly(`headers ${ JSON.stringify(req.headers, null, 2) }`);
    }
    if (req.body) {
