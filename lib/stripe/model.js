...
function Model (mock, stripe) {
  ...
  this.card = function({
    ...
    model.exp_year += model.exp_year < 2000 ? 2000 : 0;
  ...
  this.paymentMethod = function({
    ...
    const model = {
      ...
      type: 'card',
      card: card.number && !card.id ? this.card({
        context,
        card,
        type: stripe.data.cards[card.number],
        metadata,
      }) : card,
  ...
  this.charge = function({
    ...
    const model = {
      ...
      payment_intent: null,
      payment_method: card?.id || method?.id || card,
