...
function Customers (mock, stripe) {
  this.addDefaultSource = (identity, customer) => {
    if (customer.default_source) {
      const card = stripe.store.getCard(identity, customer.default_source);
      if (card) {
  ...
  this.addDefaultPaymentMethod = (identity, customer) => {
    if (customer.invoice_settings?.default_payment_method) {
      const method = stripe.store.getPaymentMethod(identity, customer.invoice_settings.default_payment_method);
      if (method) {
        customer.invoice_settings.default_payment_method = method;
      }
    }
    return customer;
  };
  ...
  this.addSources = (identity, customer) => {
    const cards = stripe.store.findCards(identity, { customer: customer.id }).sort((a, b) => {
  ...
  this.retrieveCustomer = (req, res, next) => {
    ...
    const expands = Array.isArray(req.query.expand) ? req.query.expand : [];

    if (expands.includes('subscriptions')) {
      this.addSubscriptions(context.identity, response);
    }

    if (expands.includes('invoice_settings.default_payment_method')) {
      this.addDefaultPaymentMethod(context.identity, response);
    }

    if (expands.includes('default_source')) {
      this.addDefaultSource(context.identity, response);
    }
