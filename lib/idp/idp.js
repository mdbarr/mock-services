...
function IdentityProvider (mock) {
  ...
  mock.api.get('/idp/metadata/:namespace?', async (req, res) => {
    const validUntil = new Date(Date.now() + mock.utils.milliseconds('365d')).toISOString();

    if (req.params.namespace) {
      this.log.info(`generating custom metadata for namespace ${ req.params.namespace }`);
    }
  ...
  mock.api.post('/idp/sso/:namespace?', async (req, res) => {
