...
let sessionIndex = 0;
function assertion ({
  requestId, baseUrl, namespace = '', instant, issuer, spUrl, email, name,
}) {
  ...
  return `<saml:Assertion xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" Version="2.0" ID="${ uuid() }" IssueInstant="${ instant }">
    <saml:Issuer>${ baseUrl }/${ namespace }</saml:Issuer>
    <saml:Subject>
...
function signin ({
  baseUrl, namespace = '', requestId, instant, spUrl, issuer, relay,
}) {
  return `<!doctype html>
<html lang="en-US">
  <head>
    <title>Mock Services IDP / SAML</title>
...
function metadata ({
  id, baseUrl, namespace = '', validUntil, certificate,
}) {
...
<EntityDescriptor
  ...
  <IDPSSODescriptor WantAuthnRequestsSigned="true" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
    <SingleSignOnService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect" Location="${ baseUrl }/sso/${ namespace }" />
    <SingleSignOnService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="${ baseUrl }/sso/${ namespace }" />
    <SingleSignOnService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Artifact" Location="${ baseUrl }/sso/${ namespace }" />

    <SingleLogoutService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect" Location="${ baseUrl }/slo/${ namespace }" />
    <SingleLogoutService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="${ baseUrl }/slo/${ namespace }" />
    <SingleLogoutService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Artifact" Location="${ baseUrl }/slo/${ namespace }" />

    <ArtifactResolutionService Binding="urn:oasis:names:tc:SAML:2.0:bindings:SOAP" Location="${ baseUrl }/ars/${ namespace }" index="0" />
...
function response ({
  certificate, key, requestId, issuer, baseUrl, namespace = '', instant, spUrl, email, name,
}) {
...
<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" ID="${ uuid() }" InResponseTo="${ requestId }" Version="2.0" IssueInstant="${ instant }" Destination="${ spUrl }">
  <saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">${ baseUrl }/${ namespace }</saml:Issuer>
  <samlp:Status>
