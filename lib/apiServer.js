...
function ApiServer (mock) {
  ...
  mock.api.get('/api/mock/version', (req, res, next) => {
    res.send(200, { version: mock.config.version });
    return next();
  });
  ...
  mock.api.get('/favicon.ico', serveStatic({
    directory: './dist',
    file: 'favicon.ico',
  }));

  mock.api.get('/mock-services.png', serveStatic({
    directory: './dist',
    file: 'mock-services.png',
  }));
