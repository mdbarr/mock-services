...
function DNS (mock) {
  ...
  const cacheKey = (question) => `${ question.class }-${ question.type }-${ question.name }`;
  ...
  mock.api.get('/api/mock/dns/:type(a|aaaa|cname|mx|ns|ptr|srv|txt)', (req, res, next) => {
    const type = req.params.type.toUpperCase();
    const records = this.config.records[type];

    if (!records) {
      return next(new errors.BadRequestError(new Error(`Unknown Type ${ type }`)));
    }
  ...
  mock.api.put('/api/mock/dns/:type(a|aaaa|cname|mx|ns|ptr|srv|txt)', (req, res, next) => {
    const type = req.params.type.toUpperCase();
    const records = this.config.records[type];

    if (!records) {
      return next(new errors.BadRequestError(new Error(`Unknown Type ${ type }`)));
    } else if (!mock.utils.size(req.body)) {
  ...
  mock.api.del('/api/mock/dns/:type(a|aaaa|cname|mx|ns|ptr|srv|txt)', (req, res, next) => {
    const type = req.params.type.toUpperCase();
    const records = this.config.records[type];
